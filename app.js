...
function chartHostHeight() {
  const el = document.getElementById("chart-main");
  if (!el) return CHART_HEIGHT;
  return Math.max(CHART_HEIGHT, Math.round(el.clientHeight) || CHART_HEIGHT);
}

function fitChartToHost(chart) {
  if (!chart || !chart.setSize) return;
  const next = chartHostHeight();
  if (Math.abs((chart.chartHeight || 0) - next) > 2) {
    chart.setSize(null, next, false);
  }
}

function afterChartPaint(chart) {
  requestAnimationFrame(() => {
    fitChartToHost(chart);
    if (state.view === "columns") {
      drawColumnBridges(chart);
      syncColumnAnno(chart, liveStages());
    }
    if (usesLabelOverlay()) syncLabels(chart);
  });
}
...
function render() {
  ...
  if (horizontal) {
    ...
    requestAnimationFrame(() => layoutHorizontal(host, liveStages()));
    return;
  }

  const height = chartHostHeight();
  const options =
    state.view === "current"
      ? currentOptions(stages, height)
      : state.view === "columns"
        ? columnFunnelOptions(stages, height)
        : funnelOptions(stages, height);
  ...
  if (!rebuild && existing) {
    ...
    afterChartPaint(existing);
    return;
  }
  destroyCharts();
  state.charts = [Highcharts.chart("chart-main", options)];
  afterChartPaint(state.charts[0]);
}
...
function bind() {
  ...
  document.getElementById("volume-default").addEventListener("click", () => {
